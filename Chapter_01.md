...
#### 프로그래밍 언어의 진법 표현법
1. 0으로 시작하면 8진수이다 ( ex) 017은 8진수이고 10진수로 15이다.)
2. 1~9로 시작하면 10진수이다.
3. 0x가 앞에 붙으면 16진수이다. ( ex) 0x12f는 16진수이고 10진수로 303이다)
### 비트 그룹의 이름
...
> - Base64는 HTML 또는 Email 등 문자를 위한 Media에 2진수 데이터를 포함할 때, 포함된 2진수 데이터가 안전히 전송 또는 저장되는 걸 보장하기 위해 사용
